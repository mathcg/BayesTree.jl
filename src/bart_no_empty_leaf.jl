...
function bart_options(;num_trees::Int=200,num_draws::Int=1000,
                     num_burn_in::Int=200,alpha::Float64=0.95,beta::Float64=2.0,
                     k::Int=2,proposal_probabilities=BartProbability())
   BartOptions(num_trees,num_draws,num_burn_in,alpha,beta,k,proposal_probabilities)
end
...
function update_tree!(bart_state::BartState,tree::BartTree,x::Matrix{Float64},residual::Vector{Float64},bartoptions::BartOptions)
    ...
    if update_probability<prob.node_grow_prune
      alpha,updated = node_grow_prune!(bart_state,tree,x,residual,bartoptions)
    elseif update_probability<prob.node_grow_prune+prob.change_decision_rule
      alpha,updated = change_decision_rule!(bart_state,tree,x,residual,bartoptions)
    else
      alpha,updated = swap_decision_rule!(bart_state,tree,x,residual,bartoptions)
    end
...
function node_grow_prune!(bart_state::BartState,tree::BartTree,x::Matrix{Float64},residual::Vector{Float64},bartoptions::BartOptions)
    ...
    if u<probability_grow
      alpha,updated = node_grow!(bart_state,tree,probability_grow,x,residual,bartoptions)
    else
      probability_prune = 1-probability_grow
      alpha,updated = node_prune!(bart_state,tree,probability_prune,x,residual,bartoptions)
    end
    alpha,updated
end

function leaf_available_predictors(leaf::BartLeaf,x::Matrix{Float64})
     available_predictors = Int[]
     for (i=1:size(x,2))
       feature = x[leaf.leaf_data_indices,i]
       if length(unique(feature))>1
          push!(available_predictors,i)
       end
     end
     available_predictors
end

function leaf_available_locations(leaf::BartLeaf,x::Matrix{Float64},split_feature::Int)
     available_locations = Int[]
     leaf_indices = leaf.leaf_data_indices
     feature = x[leaf_indices,split_feature]
     for i in length(feature)
       ...
       if length(left_indices)*length(right_indices)!=0
           push!(available_locations,leaf_indices[i])
        end
     end
     available_locations
end
...
function node_grow!(bart_state::BartState,tree::BartTree,probability_grow::Float64,x::Matrix{Float64},residual::Vector{Float64},bartoptions::BartOptions)
    ...
    #available_predictors = leaf_available_predictors(leaf,x)
    #uniformly sample a splitting feature from the available predictors
    #split_feature = available_predictors[rand(1:length(available_predictors))]
...
function unique_value(x::Matrix{Float64},train_data_indices::Vector{Int64},split_feature::Int)
  feature = x[train_data_indices,split_feature]
  unique_value = unique(feature)
  unique_value
end

function valid_split(x::Matrix{Float64},branch::DecisionBranch,split_value::Float64,feature::Vector{Float64})
    valid = length(feature)==0?false:true
    ...
    type_left = typeof(branch.left)==DecisionBranch?1:0
    type_right = typeof(branch.right)==DecisionBranch?1:0
    left_branch_indices = branch_indices[feature.<=split_value]
    right_branch_indices = branch_indices[feature.>split_value]
    valid = type_left==0?true:valid_split(x,branch.left,branch.left.value,x[left_branch_indices,branch.left.feature])
    if !valid
      return false
    end

    valid = type_right==0?true:valid_split(x,branch.right,branch.right.value,x[right_branch_indices,branch.right.feature])
    valid
end
...
function tree_adjust!(branch::DecisionBranch,x::Matrix{Float64},residual::Vector{Float64},branch_indices::Vector{Int64})
    ...
    tree_adjust!(branch,branch.left,x,residual,left_data_indices,true)
    tree_adjust!(branch,branch.right,x,residual,right_data_indices,false)
end
...
function initialize_bart_state(x::Matrix{Float64},y_normalized::Vector{Float64},bartoptions::BartOptions)
  ...
  for (tree in bart_state.trees)
      ...
      yhat+=predict(tree,y)-y_tree_hat
  end
  bart_state
end
...
function StatsBase.fit(x::Matrix{Float64},y::Vector{Float64},bartoptions::BartOptions)
  ...
  for i in range(1,bartoptions.num_draws+bartoptions.num_burn_in)
      ...
      for  j = 1:bartoptions.num_trees
            ...
            alpha,updated = update_tree!(bart_state,bart_state.trees[j],x,residual,bartoptions)
            updates+=updated?1:0
...
function StatsBase.predict(bart_state::BartState,x::Matrix{Float64})
  [Float64(predict(bart_state,vec(x[i,:]))) for i in range(1,size(x,1))]
end

function StatsBase.predict(tree::BartTree,x::Matrix{Float64})
   [Float64(predict(tree,vec(x[i,:]))) for i in range(1,size(x,1))]
end

function StatsBase.predict(bart_additive_tree::BartAdditiveTree,x::Matrix{Float64})
  y_predict = zeros(size(x,1))
...
function StatsBase.predict(bart::Bart,x::Matrix{Float64})
   y_predict = zeros(size(x,1),bart.options.num_draws)
   for i in range(1,bart.options.num_draws)
